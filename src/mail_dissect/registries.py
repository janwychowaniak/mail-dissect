...
class Registries:
    ...
    __slots__ = ("_exceptions", "_extensions", "_rules", "_versions", "_wildcards")
    ...
    @classmethod
    def load(cls, data_dir: Path | None = None) -> Registries:
        ...
        extensions = _parse_extensions(mime_bytes)
        versions = Versions(
            public_suffix_list=manifest["public_suffix_list"]["version"],
            file_extensions=manifest["file_extensions"]["version"],
        )
        return cls(rules, wildcards, exceptions, extensions, versions)
